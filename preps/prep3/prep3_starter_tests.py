...
def test_total_payroll_same() -> None:
    my_corp = Company([SalariedEmployee(24, 'Gilbert the cat', 1200.0),
                       SalariedEmployee(25, 'Chairman Meow', 6000.0),
                       SalariedEmployee(26, 'Paul', 12000.0)])
    my_corp.pay_all(date(2018, 6, 28))
    assert my_corp.total_payroll() == 1600.0


def test_total_pay_large_payment() -> None:
    e = SalariedEmployee(14, 'Gilbert the cat', 1200.0)
    e.pay(date(2018, 6, 28))
    e.pay(date(2018, 7, 28))
    e.pay(date(2018, 8, 28))
    e.pay(date(2018, 9, 28))
    e.pay(date(2018, 10, 28))
    e.pay(date(2018, 11, 28))
    e.pay(date(2018, 12, 28))
    e.pay(date(2019, 1, 28))
    e.pay(date(2018, 2, 28))
    assert e.total_pay() == 900.0
