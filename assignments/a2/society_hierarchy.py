...
from typing import List, Optional, TextIO, Any, final
...
class Citizen:
    ...
    def add_subordinate(self, subordinate: Citizen) -> None:
        ...
        Precondition: subordinate has no existing superior
        ...
        True
        """
        merge_list = [subordinate]
        self._subordinates = merge(self._subordinates, merge_list)
        subordinate.set_superior(self)
        

    def remove_subordinate(self, cid: int) -> None:
        """Remove the direct subordinate with the ID <cid> from this Citizen's list
        of subordinates.
    ...
    def become_subordinate_to(self, superior: Optional[Citizen]) -> None:
        ...
        []
        """
        # first remove self from its superior's subordinate list
        if self._superior is not None:
                self._superior.remove_subordinate(self.cid)

        # set self's superior to <superior> and add <self> to <superior>'s
        # subordinate list if the superior is not None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        if self._subordinates == []:
            return []
        # elif self._subordinates != []:
        #     return self._subordinates (If I call this, it would never run recursively -> need to figure out 
        # how to make this the base case like with self._items for a BST) 
        else:
            final_list = self._subordinates[:]
            for subordinate in self._subordinates:
                descendants_list = subordinate.get_all_subordinates()
                final_list = merge(final_list, descendants_list)
            return final_list
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        if self.get_citizen(cid):
            # if this citizen is cid's superior
            return self
        elif cid == self._superior.cid:
            # if this citizen's superior is cid
            return self._superior
        elif any(s.cid == cid for s in self._superior.get_all_subordinates()):
            # if this citizen's superior has cid as a subordinate
            return self._superior
        else:
            # recursive case: go up a superior and keep checking if it has cid
            # as a subordinate
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        'Starky Industries'
        """
        # define a custom function for max to use as the values to find the
        # citizen with the maximum of
        return max(self.get_direct_subordinates(), key = lambda c: c.rating)
...
class Society:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self._head is None:
            ...
            for c in self._head.get_all_subordinates() + [self._head]:
                if c.cid == cid:
                    return c
            return None
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if superior_id is None:
            if self._head is None:
                # new head to empty society
                self._head = citizen
            else:
                # new head replacing existing head
    ...
    def get_citizens_with_job(self, job: str) -> List[Citizen]:
        ...
        if self._head is None:
            ...
            return [c for c in self.get_all_citizens() if c.job == job]
    ...
    def change_citizen_type(self, cid: int,
        ...
        # create a Citizen or a DistrictLeader depending on what c is
        ...
        # move all subordinates of c to new_c
        a = c.get_direct_subordinates()
        for sub in c.get_direct_subordinates():
            sub.become_subordinate_to(new_c)

        # if c has a superior, make new_c a sub to the superior and remove
        # c as a sub to its superior
        if self._head == c:
            self._head = new_c
        else:
            superior = c.get_superior()
            superior.remove_subordinate(c.cid)
            new_c.become_subordinate_to(superior)
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        # 1. change citizen type
        # 2. change jobs
        # 3. change positions (preserve superiors and subordinates)

        superior = citizen.get_superior()
        superior_superior = superior.get_superior()
        # change citizen type if the superior is a DistrictLeader
        ...
        # remove all subordinates of <superior>
        for sub in superior_subs:
            superior.remove_subordinate(sub.cid)
        # move all subordinates of <citizen> to <superior>
        ...
        # move all previous subordinates of <superior> (not including <citizen>)
        # to <citizen>
        for sub in superior_subs + [superior]:
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        """
        c = self.get_citizen(cid)
        subordinates =  c.get_direct_subordinates()
        if self._head == c:
            # case where <c> is the head of the society
            if subordinates == []:
                # if <c> is the only citizen in the society
                self._head = None
            else:
                # if <c> is the head and also has subordinates, find the one
                # with highest rating to replace as the head
                self._head = c.get_highest_rated_subordinate()
                self._head.become_subordinate_to(None)
                # move all of <c>'s subordinates (not including the new head)
                for sub in subordinates:
                    sub.become_subordinate_to(self._head)
        else:
            superior = c.get_superior()
            # move all of <c>'s subordinates to subordinates of <superior>
            for sub in subordinates:
                sub.become_subordinate_to(superior)
            # set <c>'s superior to None, thereby removing it from the
            # hierarchy as it is not connected to any other citizen
...
class DistrictLeader(Citizen):
    ...
    def get_district_citizens(self) -> List[Citizen]:
        ...
        True
        """
        return [c for c in merge(self.get_all_subordinates(), [self])]
