...
def test_get_citizen_none() -> None:
    c1 = Citizen(1, "Pooland", 3012, "Poohead", 50)
    c2 = Citizen(2, "headass", 3000, "ass", 51)
    c3 = Citizen(3, 'Aarnavland', 2002, "Stupidfuck", 1)
    c4 = Citizen(4, "Lauraland", 2003, "Laura", 10)
    c2.add_subordinate(c1)
    c1.add_subordinate(c3)
    c2.add_subordinate(c4)
    assert c2.get_citizen(1) is c1 and c2.get_citizen(12) is None
    
def test_get_citizen_sample_society1() -> None:
    """Test get_citizen on District Leaders and Citizens"""
    s = Society()
    c1 = Citizen(1, 'Citizen 1', 3001, 'Big boss', 10)
    c2 = DistrictLeader(2, 'Citizen 2', 3002, 'Bank robber', 19, 'D2')
    c3 = Citizen(3, 'Citizen 3', 3003, 'Cook', 82)
    ...
    # c1.add_subordinate(c8)
    # c8.add_subordinate(c9)
    # c9.add_subordinate(c10)
    
    assert c1.get_citizen(8) == c8
    assert c1.get_citizen(6) == c6
    assert c1.get_citizen(15) is None
...
def test_get_society_head_complex() -> None:
    """Test get_society_head on a complex society with DistrictLeaders"""
    s = Society()
    c1 = Citizen(1, 'Citizen 1', 3001, 'Big boss', 10)
    c2 = DistrictLeader(2, 'Citizen 2', 3002, 'Bank robber', 19, 'D2')
    c3 = Citizen(3, 'Citizen 3', 3003, 'Cook', 82)
...
def test_get_closest_common_superior_complex() -> None:
    """Test get_closest_common_superior on District Leaders and Citizens"""
    s = Society()
    c1 = Citizen(1, 'Citizen 1', 3001, 'Big boss', 10)
    c2 = DistrictLeader(2, 'Citizen 2', 3002, 'Bank robber', 19, 'D2')
    c3 = Citizen(3, 'Citizen 3', 3003, 'Cook', 82)
    ...
    assert c4.get_closest_common_superior(c5) == c1
    assert c6.get_closest_common_superior(c5) == c5
...
def test_swap_up_same_type() -> None:
    s = promote_citizen_example()
    c11 = s.get_citizen(11)
    c7 = s.get_citizen(7)
    assert c11 in c7.get_direct_subordinates()
    assert c11 in c7.get_all_subordinates()

    c11 = s._swap_up(c11)
    c7 = s.get_citizen(7)
    assert c11 not in c7.get_direct_subordinates()
    assert c11 not in c7.get_all_subordinates()
    assert c7 in c11.get_direct_subordinates()
    assert c7 in c11.get_all_subordinates()
    assert c7.get_superior() == c11

def test_swap_up_citizen_to_district_leader() -> None:
    s = promote_citizen_example()
    c5 = s.get_citizen(5)
    c7 = s.get_citizen(7)
    assert c7 in c5.get_direct_subordinates()
    assert c7 in c5.get_all_subordinates()
    assert c7.get_direct_subordinates() == [s.get_citizen(11), s.get_citizen(13)]
    assert isinstance(c7, Citizen)
    assert isinstance(c5, DistrictLeader)

    c7 = s._swap_up(c7)
    c5 = s.get_citizen(5)
    print(s)
    assert c7 not in c5.get_direct_subordinates()
    assert c7 not in c5.get_all_subordinates()
    assert c7.get_direct_subordinates() == [c5]
    assert isinstance(c7, DistrictLeader)
    
    assert c5 in c7.get_direct_subordinates()
    assert c5 in c7.get_all_subordinates()
    assert c5.get_direct_subordinates() == [s.get_citizen(11), s.get_citizen(13)]
    assert isinstance(c5, Citizen)

def test_swap_up_district_leader_to_citizen() -> None:
    s = Society()
    s.add_citizen(DistrictLeader(6, "Star", 3036, "CFO", 20, "Area 52"))
    s.add_citizen(DistrictLeader(5, "S.T.A.R.R.Y Lab", 3024, "Manager", 50, "Finance"), 6)
    s.add_citizen(Citizen(7, "Hookins", 3071, "Labourer", 60), 5)
    s.add_citizen(DistrictLeader(11, "Starky", 3036, "Repairer", 90, "Marketing"), 7)
    s.add_citizen(Citizen(13, "STARRY", 3098, "Eng", 86), 7)

    c11 = s.get_citizen(11)
    c7 = s.get_citizen(7)
    assert c11 in c7.get_direct_subordinates()
    assert c11 in c7.get_all_subordinates()
    assert c11.get_direct_subordinates() == []
    assert isinstance(c7, Citizen)
    assert isinstance(c11, DistrictLeader)
    print(s)
    c11 = s._swap_up(c11)
    c7 = s.get_citizen(7)
    print(s)
    assert c11 not in c7.get_direct_subordinates()
    assert c11 not in c7.get_all_subordinates()
    assert c11.get_direct_subordinates() == [c7, s.get_citizen(13)]
    assert isinstance(c7, DistrictLeader)
    
    assert c7 in c11.get_direct_subordinates()
    assert c7 in c11.get_all_subordinates()
    assert c7.get_direct_subordinates() == []
    assert isinstance(c11, Citizen)
...
def test_promote_citizen_simple_same_type() -> None:
    s = Society()
    c1 = Citizen(1, '1', 2022, 'job1', 10)
    c2 = Citizen(2, '2', 2023, 'job2', 20)
    s.add_citizen(c1)
    assert s._head == c1
    s.add_citizen(c2, 1)
    assert s._head.get_all_subordinates() == [c2]
    print(s)

    assert s._head == c1
 
    x = s._swap_up(c2)
    assert x == s.get_citizen(2)
    assert s._head == s.get_citizen(2)
    assert s.get_citizen(2).get_all_subordinates() == [s.get_citizen(1)]
    
    c1a = s.get_citizen(1)
    c2s = s.get_citizen(2)
    assert c1a.get_all_subordinates() == []
    assert (c1a.cid, c1a.manufacturer, c1a.model_year, c1a.job, c1a.rating) == (c1.cid, c1.manufacturer, c1.model_year, c1.job, c1.rating)
...
if __name__ == '__main__':
    ...
    pytest.main(['/Users/kanishsharma/Desktop/My love Jason/csc148/assignments/a2'])
