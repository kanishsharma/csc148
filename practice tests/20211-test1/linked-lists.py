...
def test_demo_pass():
    """Test insert_one_less_before on a buggy implementation and have it pass"""
    lst = LinkedList([0, 2, 4, 6])
    lst.insert_one_less_before()
    assert str(lst) == '[-1 -> 0 -> 1 -> 2 -> 3 -> 4 -> 5 -> 6]'
    
def test_demo_fail():
    """Test insert_one_less_before on a buggy implementation and have it fail"""
    lst = LinkedList([])
    assert str(lst) == '[]'
