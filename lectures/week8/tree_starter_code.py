...
class Tree:
    ...
    #   self._subtrees does not have any empty trees 
    ...
    def leaves(self) -> List:
        ...
        [4, 5, 6, 7]
        """
        if self._root is None: # could also do if self.is_empty()
            return []
        elif self._subtrees == []:
            return [self._root]
        else:
            return_list = []
            for subtree in self._subtrees:
                return_list += subtree.leaves() #can't use .append(), it would create a list of lists, but .extend() works
                "Could also use .extend() - += doesn't create a new list so would be equally efficient"
            return return_list
    ...
    def average(self) -> float:
        ...
        if self.is_empty():
            ...
            travelled = self.traverse_tree()
            return travelled[0] / travelled[1]
    
    def traverse_tree(self) -> Tuple:
        """Return the number of nodes and the sum of all
        the values in the note as a tuple
        
        Precondition: this is a tree of numbers
        
        Returns a tuple of (sum of nodes, total # of nodes traversed)
        """
        # if self.is_empty():
        #     return (0, 0)
        if self._subtrees == []:
            ...
            total_sum = self._root
            nodes_travelled = 1
            for item in self._subtrees:
                subtree_val = item.traverse_tree()
                total_sum += subtree_val[0]
                nodes_travelled += subtree_val[1]
            return (total_sum, nodes_travelled)        
    ...
    def _delete_root(self) -> bool:
        """Delete the root of any given tree
        ...
        if self._subtrees == []:
            ...
            new_root = self._subtrees[0]
            self._root = new_root
            self._subtrees.remove(new_root)
